...
const MOB_COLORS: Record<string, number> = { slime: 0x5fd068, wolf: 0x9aa5b8, golem: 0xb08050 };
...
export class EntityView {
  ...
  private build(isSelf: boolean): void {
    ...
    switch (s.kind) {
      ...
      case 'mob': {
        const c = MOB_COLORS[s.mobType ?? 'slime'] ?? 0xcccccc;
        if (s.mobType === 'golem') {
          const pts: number[] = [];
          for (let i = 0; i < 6; i++) {
      ...
      case 'boss': {
        g.circle(0, 0, r + 16).fill({ color: 0xb565d8, alpha: 0.15 });
        g.circle(0, 0, r + 8).fill({ color: 0xb565d8, alpha: 0.25 });
        g.circle(0, 0, r).fill(0x7a2f9e).stroke({ width: 4, color: 0xf0c8ff, alpha: 0.8 });
        for (let i = 0; i < 8; i++) {
        ...
        const name = new Text({ text: '💀 БОСС', style: { fontFamily: 'system-ui', fontSize: 16, fontWeight: '800', fill: 0xf0c8ff, stroke: { color: 0x000000, width: 4 } } });
        name.anchor.set(0.5);
