...
| 4.4 | реклама только после неигрового действия | ролик выходит по нажатию «Возродиться» на экране смерти — это и есть пример из документации Яндекса («после проигрыша и нажатия кнопки Снова»). Сама смерть рекламы не вызывает: это не действие игрока |
...
| 1.10.3 | элементы интерфейса не накладываются друг на друга | HUD разложен заново под все формы окна, включая короткое широкое — телефон боком в кадре Яндекса; проверяется попарным пересечением всех рамок в `npm run overlap` |
...
| `npm run ads` | доводит актёра до смерти и проверяет: сама смерть рекламы не вызывает, ролик выходит по нажатию «Возродиться» и не позже 0,33 с после него, герой всё это время остаётся мёртвым, игра стоит всё время показа вместе с предупреждением, и возрождается он только после закрытия (пп. 4.4, 4.7) |
| `npm run overlap` | замеряет каждый элемент HUD в семи формах окна (в том числе 730×310 — телефон боком) и пересекает все пары: ни одного наложения, ничего за краем, магазин влезает, в режиме стройки тоже (п. 1.10.3) |
...
- **Реклама (п. 4.4)** — полноэкранная выходит по нажатию «Возродиться» на
  экране смерти и никогда сама по себе. Смерть — не действие игрока, поэтому на
  неё ничего не вешается; просьба вернуться в бой — действие, а экран смерти —
  та самая логическая пауза. Запрос уходит в том же обработчике нажатия, то есть
  заведомо внутри разрешённых 0,33 с. Возрождение ждёт закрытия ролика: игра
  продолжается только после того, как экран снова наш.
