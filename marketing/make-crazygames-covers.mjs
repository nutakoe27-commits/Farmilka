...
//   node marketing/render-svg.mjs            # rasterises them to PNG
//
// The scene is the pitch: a walled base with a vault full of gold on one side,
// someone breaking in on the other. Shapes and colours are lifted straight
// from the game's own rendering (client/src/game/entities.ts) so the cover and
// the first second of gameplay look like the same product.
...
/** "Build your farm." in white / "Raid theirs." in red — the whole pitch in one line. */
const tagline = (x, y, size) =>
  `<text x="${x}" y="${y}" text-anchor="middle" font-size="${size}" font-weight="700">` +
  `<tspan fill="#c7cedb">Build your farm. </tspan><tspan fill="${C.raider}">Raid theirs.</tspan></text>`;
...
/**
 * The raid scene, parameterised so all three orientations share one composition:
 * the base and its keeper on the left, a wall line down the middle, a raider
 * smashing through it on the right. The impact burst is the focal point.
 */
function raidScene({ cx, cy, s, wallCount = 5 }) {
  const step = 104 * s;
  const wx = cx + 210 * s;
  const walls = [];
  const mid = Math.floor(wallCount / 2);
  for (let i = 0; i < wallCount; i++) {
    walls.push(wall(wx, cy + (i - (wallCount - 1) / 2) * step, 46 * s, i === mid ? 0.78 : 0));
  }
  return `
  <circle cx="${cx - 120 * s}" cy="${cy}" r="${420 * s}" fill="#1f9c4b" opacity="0.22" filter="url(#soft)"/>
  <circle cx="${cx + 470 * s}" cy="${cy}" r="${340 * s}" fill="#e0574f" opacity="0.22" filter="url(#soft)"/>
  ${farm(cx - 300 * s, cy - 150 * s, 54 * s, 0.9)}
  ${farm(cx - 320 * s, cy + 170 * s, 48 * s, 0.5)}
  ${vault(cx - 60 * s, cy, 76 * s)}
  ${player(cx - 190 * s, cy + 26 * s, 42 * s, C.defender, 8, bow)}
  ${walls.join('')}
  ${burst(wx - 62 * s, cy, 70 * s)}
  ${player(cx + 400 * s, cy, 50 * s, C.raider, 180, hammer)}
  ${coin(cx + 90 * s, cy + 196 * s, 30 * s)}
  ${coin(cx + 176 * s, cy + 250 * s, 22 * s)}
  ${coin(cx + 26 * s, cy - 214 * s, 25 * s)}
  ${coin(cx + 250 * s, cy - 186 * s, 19 * s)}`;
}

// ---- landscape 16:9 ----
const landscape = svg(1920, 1080, `
  ${raidScene({ cx: 900, cy: 400, s: 1.12, wallCount: 5 })}
  ${logo(300, 800, 1.5)}
  ${TITLE(1020, 850, 168)}
  ${tagline(960, 950, 56)}
  ${sub(960, 1020, 38, '#8a93a5', 600, 'Multiplayer .io base raider')}
`);

// ---- portrait 9:16 ----
const portrait = svg(1080, 1920, `
  ${raidScene({ cx: 470, cy: 760, s: 0.9, wallCount: 6 })}
  ${logo(540, 1300, 2.2)}
  ${TITLE(540, 1560, 152)}
  ${tagline(540, 1660, 52)}
  ${sub(540, 1740, 40, '#8a93a5', 600, 'Multiplayer .io base raider')}
`);

// ---- square 1:1 ----
const square = svg(1080, 1080, `
  ${raidScene({ cx: 470, cy: 400, s: 0.62, wallCount: 5 })}
  ${logo(540, 700, 1.4)}
  ${TITLE(540, 870, 114)}
  ${tagline(540, 950, 40)}
  ${sub(540, 1010, 30, '#8a93a5', 600, 'Multiplayer .io base raider')}
`);
